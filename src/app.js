...
function analysisPage(selectedTag='',folderId='',isThinking=false){
 ...
 if(folderId&&!folder)return head('404 / NOT FOUND','文件夹未找到','这个文件夹不存在或尚未公开。')+'<a class="back" href="#/'+section+'">← 返回</a>';
 ...
 const cards=filtered.map(a=>{
  ...
  return '<li><a class="analysis-post" href="#/'+(isFolder?folderRoute:articleRoute)+'/'+encodeURIComponent(a.id)+'">'+
   ...
   '<div class="analysis-post-meta">'+(isFolder?'<span><span class="meta-icon icon-folder" aria-hidden="true"></span> '+a.articleCount+' 篇文章</span><span>文件夹</span>':'<span><span class="meta-icon icon-clock" aria-hidden="true"></span> '+Math.max(1,a.readingMinutes||1)+' min</span><span><span class="meta-icon icon-globe" aria-hidden="true"></span> zh-CN</span>')+
 ...
 const back=folder?'<a class="analysis-breadcrumb" href="'+(folder.parent?'#/'+folderRoute+'/'+encodeURIComponent(folder.parent):'#/'+section)+'">← 返回</a>':'<a class="analysis-breadcrumb" href="#/">← 首页 <span>/ 02</span></a>';
...
function detail(item,isProject,isThinking=false){
 ...
 if(!item)return head('404 / NOT FOUND','文章未找到','这篇文章不存在或尚未公开。')+'<a class="back" href="#/'+section+'">← 返回</a>';
 ...
 return '<div class="reading-page"><a class="reading-back" href="'+back+'">‹ 返回</a><div class="reading-layout">'+
 '<article class="reading-main">'+cover+'<header class="reading-header"><div class="reading-meta">'+(date?'<span><span class="meta-icon icon-calendar-days" aria-hidden="true"></span> <time datetime="'+esc(item.date)+'">'+esc(date)+'</time></span>':'')+
 '<span><span class="meta-icon icon-clock" aria-hidden="true"></span> '+Math.max(1,item.readingMinutes||1)+' min</span><span><span class="meta-icon icon-globe" aria-hidden="true"></span> zh-CN</span>'+
 (item.tags?.length?'<span class="reading-tags"><span class="meta-icon icon-tag" aria-hidden="true"></span> '+item.tags.map(t=>isProject?esc(t):'<a href="#/'+section+'/'+encodeURIComponent(t)+'">'+esc(t)+'</a>').join(' / ')+'</span>':'')+'</div>'+
 ...
 (toc?'<aside class="reading-toc"><h2>TABLE OF CONTENTS</h2><nav aria-label="文章目录">'+toc+'</nav></aside>':'')+'</div></div>';
}
function about(){
